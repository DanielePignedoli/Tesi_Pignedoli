...
output_name = '../data/both_embeddings.pkl'
n_coms=10 #number of community to consider

#load data
print('loading data')
df = pd.read_pickle(data_filename)
graph = ig.Graph.Read_GraphML("../graph/G_weighted.graphml")
...
def closeness(user_id, community_index, graph):
    node_index = id2index[user_id] 
    first_neighbors = graph.neighbors(node_index)
    weighted_first_neighbors = []
    for neighbor in first_neighbors:
        weight = graph.es[graph.get_eid(node_index, neighbor)]['weight']
        weighted_first_neighbors.extend([neighbor] * int(weight)) #aggiunge un numero x di elementi x = peso del link

    neigh_in_community = [membership[node] for node in weighted_first_neighbors if membership[node] == community_index]
    
    return len(neigh_in_community)/len(weighted_first_neighbors)
...
df['proximity'] = df['user.id'].apply(lambda x: [closeness(x, i, graph) for i in top_com_indeces])
